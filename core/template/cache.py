...
class HtmlToFileIdCache:
    ...
    async def get_data(self, html: str) -> Optional[str]:
        data = await self.client.get(self.cache_key(html))
        if data:
            return data.decode()

    async def set_data(self, html: str, file_id: str, ttl: int = 60 * 60):
        ck = self.cache_key(html)
        await self.client.set(ck, file_id)
        if ttl != -1:
            await self.client.expire(ck, ttl)

    def cache_key(self, html: str) -> str:
