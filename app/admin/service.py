...
class BotAdminService:
    ...
    async def get_admin_list(self) -> List[int]:
        admin_list = await self.cache.get_list()
        if len(admin_list) == 0:
            admin_list = await self.repository.get_by_user_id()
            for config_admin in config.ADMINISTRATORS:
                admin_list.append(config_admin["user_id"])
            await self.cache.set_list(admin_list)
        return admin_list

    async def add_admin(self, user_id: int) -> bool:
        try:
            await self.repository.add_by_user_id(user_id)
...
class GroupAdminService:
    ...
    async def get_admins(self, bot: Bot, chat_id: int, extra_user: List[int]) -> List[int]:
        admin_id_list = await self.cache.get_chat_admin(chat_id)
        if len(admin_id_list) == 0:
            ...
            await self.cache.set_chat_admin(chat_id, admin_id_list)
