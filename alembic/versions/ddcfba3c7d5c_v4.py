...
def upgrade() -> None:
    ...
    for old_cookies_database_name in (old_cookies_database_name1, old_cookies_database_name2):
        try:
            statement = "SELECT * FROM %s" % old_cookies_database_name
        ...
        if old_cookies_table_data is None:
            logger.error("Old Cookies Database is None")
            continue
        for row in old_cookies_table_data:
            try:
                ...
                logger.error("process %s->cookies Exception", old_cookies_database_name, exc_info=exc)
    ...
    if old_user_table_data is not None:
        for row in old_user_table_data:
            try:
                ...
                logger.error("process admin->users Exception", old_cookies_database_name, exc_info=exc)
    else:
        logger.error("Old User Database is None")
