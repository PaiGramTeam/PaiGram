...
class BeyondGachaLogMigrate(IMigrateData, BeyondGachaLog):
    old_user_id: int
    new_user_id: int
    old_uid_list: List[int]

    async def migrate_data_msg(self) -> str:
        return f"{len(self.old_uid_list)} 个账号的颂愿抽卡记录数据"
    ...
    async def migrate_data(self) -> bool:
        ...
        if uid_list:
            raise MigrateDataException(f"颂愿抽卡记录数据迁移失败：uid {','.join(uid_list)}")
        return True
