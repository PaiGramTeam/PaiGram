...
for char in AssetsService.avatar.get_instance().all_items:
    GCSIM_CHARACTER_TO_CHARACTER[from_character_gcsim_character(char.en_name)] = (char.id, char.en_name)
...
for _weapon in AssetsService.weapon.get_instance().all_items:
    GCSIM_WEAPON_TO_WEAPON[remove_non_words(_weapon.en_name.lower())] = (_weapon.id, _weapon.en_name)
...
for _artifact in AssetsService.artifact.get_instance().all_items:
    GCSIM_ARTIFACT_TO_ARTIFACT[remove_non_words(_artifact.en_name.lower())] = (_artifact.id, _artifact.en_name)
