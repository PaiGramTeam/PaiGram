    ...
    async def command_start(self, update: Update, context: CallbackContext) -> None:
        ...
        try:
            ...
            if filters.ChatType.GROUPS.filter(message):
                self.add_delete_message_job(reply_message, delay=60)
    ...
    async def set_wish(self, update: Update, context: CallbackContext) -> None:
        ...
        try:
            ...
            if filters.ChatType.GROUPS.filter(reply_message):
                self.add_delete_message_job(message, delay=10)
                self.add_delete_message_job(reply_message, delay=10)
            return
        banner = await self.get_banner(gacha_base_info)
        up_weapons = {}
